...
public class AdminMenuController {
  ...
  ResponseEntity<ApiResult> getItems() {
    if (List.of(900, 999).contains(AuthenticationUtils.getLoginVO().getAuthority())) {
      return Result.ok(this.tableMenuRepository.getItemsWithOrder(List.of("displayOrder", "name")));
    } else {
      return Result.ok(
          this.tableMemberMenuRepository.getItemsByKeyWithOrder(
              Map.of("authority", AuthenticationUtils.getLoginVO().getAuthority()),
  ...
  public ResponseEntity<ApiResult> save(@RequestBody final List<TableMenuEntity> menus) {
    menus.forEach(menu -> this.tableMenuRepository.updateByKey(menu, Map.of("id", menu.getId())));
    return Result.created();
  }
}
