...
public class AuthService implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(final String username) throws UsernameNotFoundException {
    ...
    return this.tableMemberRepository
        .getItemByKey(Map.of("id", username))
        .map(
            tableMemberEntity ->
                new UserVO(
        ...
        .map(
            tableMemberEntity -> {
              ...
              this.tableMemberRepository.updateMapByKey(
                  Map.of("token", refreshToken), Map.of("id", id));
  ...
  void logout() {
    this.tableMemberRepository.updateMapByKey(
        Map.of("token", ""), Map.of("id", AuthenticationUtils.getUserPk()));
  }

  String getNewAccessToken(final String refreshToken) {
    return this.tableMemberRepository
        .getItemByKey(Map.of("token", refreshToken, "id", JwtTokenProvider.getUserPk(refreshToken)))
        .map(
            tableMemberEntity -> {
              final UserVO userVO = MapperUtils.toObject(tableMemberEntity, UserVO.class);
    ...
    this.tableMemberRepository
        ...
        .ifPresentOrElse(
            tableMemberEntity -> {
              ...
              this.tableMemberRepository.updateMapByKey(
                  Map.of("password", password), Map.of("id", id));
            },
            () -> {
              // 1. 유저가 없으면
