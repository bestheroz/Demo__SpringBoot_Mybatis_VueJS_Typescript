...
public class DbTableVOCheckerContext {
    ...
    public static final Set<String> STRING_JDBC_TYPE_SET = Set.of("VARCHAR", "VARCHAR2", "CHAR", "CLOB");
    public static final Set<String> NUMBER_JDBC_TYPE_SET = Set.of("INTEGER", "TINYINT", "INT", "INT UNSIGNED", "NUMBER", "DECIMAL", "DECIMAL UNSIGNED", "BIGINT UNSIGNED", "BIGINT");
                    ...
                    if (!isInvalid) {
                        ...
                        for (int i = 0; i < metaInfo.getColumnCount(); i++) {
                            ...
                            if (fieldsSet.contains(camelColumnName)) {
                                ...
                                try {
                                    fieldClassName = class1.getDeclaredField(camelColumnName).getType().getSimpleName();
                                } catch (final Throwable e) {
                                    fieldClassName = class1.getSuperclass().getDeclaredField(camelColumnName).getType().getSimpleName();
                                }
                                final String columnTypeName = metaInfo.getColumnTypeName(i + 1);
                                if (STRING_JDBC_TYPE_SET.contains(columnTypeName) && !SqlCommand.VARCHAR_JAVA_TYPE_SET.contains(fieldClassName)
                                        || NUMBER_JDBC_TYPE_SET.contains(columnTypeName) && !SqlCommand.NUMBER_JAVA_TYPE_SET.contains(fieldClassName)
                                        || DATETIME_JDBC_TYPE_SET.contains(columnTypeName) && !SqlCommand.TIMESTAMP_JAVA_TYPE_SET.contains(fieldClassName)
                                        || BOOLEAN_JDBC_TYPE_SET.contains(columnTypeName) && !SqlCommand.BOOLEAN_JAVA_TYPE_SET.contains(fieldClassName)
                                        || BYTE_JDBC_TYPE_SET.contains(columnTypeName) && !SqlCommand.BLOB_JAVA_TYPE_SET.contains(fieldClassName)) {
