...
public class DbTableVOCheckerContext {
  ...
  public static final Set<String> DATETIME_JDBC_TYPE_SET = Set.of(
    "TIMESTAMP",
    "DATE",
    "DATETIME"
  );
  ...
  private Set<Class<?>> findMyTypes()
    ...
    for (final Resource resource : resourcePatternResolver.getResources(
      ...
      if (resource.isReadable()) {
        final MetadataReader metadataReader = metadataReaderFactory.getMetadataReader(
          resource
        );
  ...
  public void validDbTableVO(final SqlSession sqlSession) {
    try (
      final Statement stmt = new SqlSessionFactoryBuilder()
      ...
      for (final Class<?> class1 : targetClassList) {
        final String tableName = SqlCommand.getTableName(
        ...
        try (
          ...
          final Set<String> fieldsSet = Arrays
            .stream(
              ArrayUtils.addAll(
                class1.getSuperclass().getDeclaredFields(),
          ...
          if (isInvalid) {
            final StringBuilder voSb = new StringBuilder(
              className + ".java를 아래값으로 동기화 해주세요.\n"
            );
            for (int i = 0; i < metaInfo.getColumnCount(); i++) {
              ...
              if (STRING_JDBC_TYPE_SET.contains(columnTypeName)) {
                ...
                StringUtils.equalsAny(columnTypeName, "NUMBER", "DECIMAL")
              ) {
